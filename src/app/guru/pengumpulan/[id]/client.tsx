...
export function PengumpulanDetailClient({ judulTugas, rows }: { judulTugas: string; rows: PengumpulanRow[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">{judulTugas}</h1>
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              ...
              {table.rows.map((row) => (
                <TableRow key={row.id_pengumpulan} className="hover:bg-accent/40">
                  <TableCell>{row.nama_siswa}</TableCell>
                  <TableCell>
                    <Badge variant={row.status === "dinilai" ? "default" : "secondary"}>
      ...
      {selected && (
        <NilaiModal
          ...
          idPengumpulan={selected.id_pengumpulan}
