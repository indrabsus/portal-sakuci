...
export default async function CetakSertifikatPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: sertifikat }, { data: sekolah }] = await Promise.all([
    supabase
      .from("sertifikat")
      .select(
        "id_sertifikat, id_siswa, id_kompetensi, judul_manual, nomor_sertifikat, nilai, tanggal_terbit, kode_verifikasi, nama_kajur, jabatan_kajur, nip_kajur, diterbitkan_oleh, nama_kepsek, nip_kepsek, siswa(nama_lengkap), kompetensi(judul, syarat_lulus)",
      )
      .eq("id_sertifikat", id)
      .single(),
